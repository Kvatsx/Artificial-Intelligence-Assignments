...
class AntColonyOptimization:
    ...
    def Initialize(self):
        ...
        self.EdgePheromone = [0] * len(self.GraphEdges)
        self.antPopulate()
    ...
    def NextCity(self, index):
        ...
        CurrentAnt.updateLocation(self.GraphEdges[1])
    ...
    def updatePheromone(self, pheromone):
        for i in range(self.N):
            index = pheromone[i][1]
            self.EdgePheromone[index] = pheromone[i][0]
    ...
    def ShowSolution(self):
        newGraph = []
        for i in range(len(self.GraphEdges)):
            newGraph.append((self.GraphEdges[i][0], self.GraphEdges[i][1], self.EdgePheromone[i]))
        Populate.ShowGraph(nx.Graph(), newGraph)
